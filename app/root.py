        ...
        for user_id in user_ids:
            try:
                ...
                if user:
                    ...
                    if user.status in ['Уважаемый', 'Адепт', 'Амбассадор','Блат']:
...
async def process_new_status(message: types.Message, state: FSMContext):
    ...
    valid_statuses = ['Базовичок', 'Уважаемый', 'Адепт', 'Амбассадор','Блат']
...
async def edit_points(message: types.Message, state: FSMContext):
    await message.answer("Введите ID пользователя и количество поинтов\nНапример если вы хотите прибавить напишите 0 +100\nЕсли хотите вычесть то 0 -100\n 0 - ID, 100 - число поинтов ")
...
@is_admin()
async def process_point_edit(message: types.Message, state: FSMContext):
    # Получаем текст сообщения
    text = message.text.strip()
    
    # Проверяем, соответствует ли текст формату "ID операции"
    try:
        user_id, points_change = text.split()
        user_id = int(user_id)
        points_change = int(points_change)
    except ValueError:
        await message.answer("Неверный формат. Пожалуйста, используйте формат: ID +число или ID -число.")
        return
    ...
    async with async_session() as session:
        ...
        user.point += points_change  # Изменяем знак
        user.all_point += points_change  # Изменяем знак

        # Сохраняем изменения в базе данных
        await session.commit()

        await message.answer(f"Успешно обновлены поинты пользователя {user.name}. Текущие поинты: {user.point}")
...
@router_root.message(F.text == 'Узнать всю информацию о пользователе')
@is_admin()
async def edit_points(message: types.Message, state: FSMContext):
...
@router_root.message(UserInfo.waiting_for_user_id)
@is_admin()
async def process_point_edit(message: types.Message, state: FSMContext):
    # Получаем текст сообщения
    user_id = message.text.strip()
    ...
    async with async_session() as session:
        ...
        if users:
            user_list = 'Информация о пользователе\n\n'
            for user in users:
                user_list += f"ID :{user.id}. | Имя:{user.name} | Поинты:{user.point} | Рекорд по поинтам:{user.all_point} | Уровень: {user.level} | Кол-во посещений: {user.visit} | Администратор: {user.root} | Статус: {user.status}\n"
        else:
            user_list = 'Пользователь не найден'
        await message.reply(user_list,reply_markup=kb.back)
