...
async def show_tasks_day(message: types.Message):
    await message.answer('Выберите задание для подтверждение',reply_markup= rq.task_category_keyboard)
...
#             response = ''
#             for task in tasks:
#                 response += f" {task.id} | {task.description} | Очки: {task.point}\n\n"
#             await message.answer(response)
#             await message.answer('Напишите номер задание,что бы потдверить его выполнение')
#         else:
#             await message.answer("Задач пока нет.Приходи позже")
